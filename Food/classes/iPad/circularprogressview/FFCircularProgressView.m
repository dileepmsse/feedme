...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        self.backgroundColor = [UIColor navBarColor];
        self.alpha = 0.6667;
        
        _lineWidth = fmaxf(self.frame.size.width * 0.050, 1.f);
        _tintColor = [UIColor ios7Gray];
        ...
        _progressLayer.strokeColor =[UIColor ios7Blue].CGColor;// _tintColor.CGColor;
...
//    try piechart style progress
    CGPoint center = CGPointMake(rect.origin.x + floorf(rect.size.width / 2),
...
- (void) drawBackgroundCircle:(BOOL) partial {
    ...
    CGFloat endAngle = (2 * (float)M_PI) + startAngle;
    CGPoint center = CGPointMake(self.bounds.size.width/2, self.bounds.size.height/2);
    CGFloat radius = (self.bounds.size.width - _lineWidth*3) / 2.0;//(self.bounds.size.width - _lineWidth)/2;
